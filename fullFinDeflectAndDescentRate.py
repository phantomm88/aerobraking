...
def g(state, d_ind, A_para):
  atmo = Atmosphere(state[0][0])
  return np.array([[state[1][0]],
                   [0.5 * atmo.density[0] * state[1][0]**2 * (0.0080008654 + 4 * 0.01482401 * np.sin(np.radians(deflections[d_ind]))) * cds[d_ind] / M + 0.5 * atmo.density[0] * state[1][0]**2 * A_para * 0.97 / M - atmo.grav_accel[0]]])
...
def g_1(state, d_ind, A_para):
  return g(state, d_ind, A_para)

def g_2(state, d_ind, h, A_para):
  return g(state + 0.5 * h * g_1(state, d_ind, A_para), d_ind, A_para)

def g_3(state, d_ind, h, A_para):
  return g(state + 0.5 * h * g_2(state, d_ind, h, A_para), d_ind, A_para)

def g_4(state, d_ind, h, A_para):
  return g(state + h * g_3(state, d_ind, h, A_para), d_ind, A_para)
...
def update_state2(state, d_ind, h, A_para):
  return state + h * ((1/6) * g_1(state, d_ind, A_para) + (1/3) * g_2(state, d_ind, h, A_para) + (1/3) * g_3(state, d_ind, h, A_para) + (1/6) * g_4(state, d_ind, h, A_para))
...
def rk4(init_state, t0, xf, d_ind, A_para):
  probe_states = []
  probe = init_state
  ...
  while probe[0][0] > xf:
    ...
    if probe_i[0][0] < 200:
      probe = update_state2(probe_i, d_ind, h, A_para)
    else:
...
# parachute diameters
feets = [2.5, 2.75, 3, 3.25, 3.5, 3.75, 4, 4.25, 4.5]
areas = [x / 10.764 for x in feets]

def plotter(area_current):
  fig, (ax1, ax2) = plt.subplots(1,2, figsize=(20,8))
  for i in range(7, len(deflections)):
    ts, positions, velocities = rk4(probe_init, 0, 0, i, area_current)
    ...
    ax1.set_title('Effect of Fin Deflection on Descent Position for A_para of '+str(area_current*10.764))
    ax2.set_title('Effect of Fin Deflection on Descent Rate for A_para of '+str(area_current*10.764))
